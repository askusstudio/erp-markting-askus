...
function updateTheme() {
  ...
  dirs.forEach(dir => {
    ...
    walkDir(fullPath, (filePath) => {
      if (filePath.endsWith('.tsx') || filePath.endsWith('.ts')) {
        ...
        // Success color (emerald -> teal for pastel feel)
        ...
        // Error/Warning color (red -> rose for pastel feel)
        ...
        // Backgrounds (warm light pastel)
        content = content.replace(/bg-zinc-50/g, 'bg-[#fcfaf8]');
        
        // Borders to softer slate
        content = content.replace(/border-zinc-200/g, 'border-slate-200');
        content = content.replace(/border-zinc-300/g, 'border-slate-300');
        
        // Text to slate for softer feel instead of harsh zinc
  ...
  // Also update globals.css to remove dark mode and add pastel background
  const cssPath = path.join(__dirname, 'app', 'globals.css');
  if (fs.existsSync(cssPath)) {
    let cssContent = fs.readFileSync(cssPath, 'utf8');
    
    // Remove dark media query
    cssContent = cssContent.replace(/@media \(prefers-color-scheme: dark\) \{[\s\S]*?\}/, '');
    
    // Change background to pastel off-white
    cssContent = cssContent.replace(/--background: #ffffff;/, '--background: #fcfaf8;');
...
updateTheme();
console.log('Theme updated to light pastel successfully!');
